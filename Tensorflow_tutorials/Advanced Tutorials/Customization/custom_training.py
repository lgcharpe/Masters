...
lo = loss(model, features, labels, training=False)
print(f"Loss test: {lo}")
...
for epoch in range(num_epochs):
    ...
    # Training the loop - using batches of 32
...
for (x, y) in test_dataset:
    ...
    # behaviour during training versus testing (e.g. Dropout)
...
# Usiing the trained model to do predictions
...
# training=False is needed only if there are layers with different behaviour
# during training versus testing (e.g. Dropout)
...
for i, logits in enumerate(predictions):
    ...
    print(f"Example {i} prediction: {name} ({p:4.1%})")
